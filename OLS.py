...
print('\n1.1) Parameters: ', results.params)
print('1.2) R2: ', results.rsquared)


#OLS non-linear curve but linear in parameters
#We simulate artificial data with a non-linear relationship between x and y:
print("\n2) OLS non-linear curve but linear in parameters")
nsample = 50
sig = 0.5
...
print('\n2.1) Parameters: ', res.params)
print('2.2) Standard errors: ', res.bse)
print('2.3) Predicted values: ', res.predict())
...
ax.plot(x, iv_l, 'r--')
ax.legend(loc='best');
plt.title("Compare the true relationship to OLS predictions")
plt.show()
...
print("\n4) Joint hypothesis test")
print("\nF test")
# F test
# We want to test the hypothesis that both coefficients on the dummy variables are equal to zero, that is,  R×β=0 . 
...
# We can also look at formal statistics for this such as the DFBETAS
# -- a standardized measure of how much each coefficient changes when that observation is left out.
infl = ols_results.get_influence()


#In general we may consider DBETAS in absolute value greater than  2/√N  to be influential observations
2./len(X)**.5
